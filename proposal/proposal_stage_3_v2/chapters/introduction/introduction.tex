...
Recent advances \citep{rombach2022high, BetkerImprovingIG} in text-to-image diffusion models have significantly improved the ability to generate highly realistic images.
These capabilities have rapidly extended beyond vision to domains such as speech \citep{chen2020wavegrad}, medical image reconstruction \citep{metal_inpainting_cbct_projections, high_frequency_space_diffusion_model}, and natural language processing \citep{structured_denoising,diffusion_lm, argmax_flows}.
Together, these developments position diffusion models as a general and increasingly universal framework for generating and manipulating complex data across modalities.
...
In low-resource clinical settings, these capabilities are particularly valuable because high-quality data are often scarce or expensive to obtain \citep{GROGER2025100220, Nyamawe2026OnTU}. 
...
A generative model that can produce anatomically plausible rare cases could therefore support critical downstream tasks through data augmentation \citep{HOLSTE2024103224}.
...
The ``logical composition'' defined by the mathematical operators often diverge from the ``semantic composition'' expressable by a single prompt. 
In practice, this leads to failures such as incorrect attribute binding and interference between these concepts \citep{t2i_compbench_2025}. 
...
This suggests that the key issue is not only how composition is performed, but also how the underlying representations in the individual expert models are organised.
...
Prior work on disentangled representation learning \citep{challeau2019challenging, traeuble2021disentangled} shows that recovering disentangled factors in unsupervised settings is fundamentally ill-posed without suitable inductive biases.
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{|>{\centering\arraybackslash}m{0.13\textwidth}|>{\raggedright\arraybackslash}p{0.37\textwidth}|>{\raggedright\arraybackslash}X|}
        ...
        \textbf{Phase} & \textbf{Objective} & \textbf{Hypotheses} \\
        \hline
        ...
        \begin{minipage}[t]{\hsize}
            \vspace{1pt}
            \begin{enumerate}[leftmargin=*, nosep]
                \item Known generative factors in dSprites \citep{dsprites17} and correlated dSprites provide a valid controlled test bed for held-out composition.
        ...
        \begin{minipage}[t]{\hsize}
            \vspace{1pt}
            \begin{enumerate}[leftmargin=*, nosep]
                \item Shared anatomical structure and disease-specific effects can be represented as separable components in the latent space.
                \item Diseases can be composed from sufficiently factorized latent components.
            \end{enumerate}
            \vspace{1pt}
        \end{minipage} \\
        \hline
